...

{
  ...
  deploy.params.lan.mac = "16:07:77:0c:aa:ff";
  deploy.params.lan.ipv4 = "10.11.1.221/24";

  deploy.node.hostname = "127.0.0.1";
  deploy.node.fastConnection = true;
  deploy.node.sshUser = "admin";

  systemd.network.networks.wifi-generic.matchConfig.Name = "wl*";
  systemd.network.networks.lan.dhcpV4Config.UseRoutes = false; # FIXME: candidate for deploy module
  systemd.network.networks.local-eth.matchConfig.Name = "en*";
  ...
  systemd.network.networks.lan.addresses = [{ addressConfig.Address = "10.11.1.221/24"; }];
