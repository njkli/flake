...
let
  ...
  */
  ...
  sbcl = prev.lispPackages_new.sbclWithPackages (pkgs: with pkgs; [
    cl-diskspace
    cl-mount-info
    xml-emitter
    xkeyboard
  ...
  stumpwm-git-new = prev.lispPackages_new.sbclWithPackages (pkgs: with pkgs;
  ...
  stumpwmCustomBuild_new_lispPackages = prev.lispPackages_new.build-asdf-system {
    buildInputs = with prev; [ autoconf automake makeWrapper ];
    lisp = "${sbcl}/bin/sbcl --script";
    systems = [ "stumpwm" "dynamic-mixins" ];
    asds = [ "stumpwm" ];
    inherit (final.sources.stumpwm-git) src version pname system asd;
